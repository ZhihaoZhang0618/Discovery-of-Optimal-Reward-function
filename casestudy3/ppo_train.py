...
if __name__ == "__main__":
    ...
    run_name = f"casestudy3__ppo__{args.reward_mode}__{args.seed}__{int(time.time())}"
    ...
    if (env.train):
        ...
        for epoch in range(1, args.number_epochs):
            ...
            while ((not game_over) and timestep <= args.max_steps):
                ...
                ppo.store_transition(np.asarray(current_state), action, reward_train, np.asarray(next_state), logprob, game_over)
